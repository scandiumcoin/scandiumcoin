...
#if defined(NDEBUG)
# error "Scandiumcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4aa47377ab6d7b69bd9063962c4178549dfb38d362903534b76fc933b90fc157");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Scandiumcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Scandiumcoin Signed Message:\n";
...
{
    // Scandiumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525000); // Scandiumcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Scandiumcoin: 3 days
static const int64 nTargetSpacing = 3 * 60; // Scandiumcoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Scandiumcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Scandiumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbc3b7ddba8254dd88d34c10e6f947e0b8539d8040a77767542568afc2dd415e9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "9/27/2020 - Scandium hits over a ten million views!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fe416dbcc3a4e2571a38f0bca461aef1164ed688374ed75d2b34ae32a4c0f21f4516b385bd3869b303588670284195bdf3f74f0838d4901f09386a5bbdfe82f2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1601198937;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084756654;

        if (fTestNet)
        {
            block.nTime    = 1601198883;
            block.nNonce   = 385516670;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc8dfeaa752b360c282581706132ef3ed43ea64c0cd61de78759416c139c69634"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc3, 0xb3, 0xdd }; // Scandiumcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ScandiumcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ScandiumcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ScandiumcoinMiner(CWallet *pwallet)
{
    printf("ScandiumcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("scandiumcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ScandiumcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ScandiumcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ScandiumcoinMiner, pwallet));
}
